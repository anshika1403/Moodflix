...
def fetch_movie_details(movie_id):
    ...
    # ✅ Use 'image' instead of 'poster_path' to match frontend expectations
    ...
    return {
        ...
        'overview': overview,
        'image': image  # ✅ Use 'image' key to match frontend
    }
...
def get_content_based_recommendations(user_id):
    ...
    # ✅ Fetch additional movies to compare similarity
    additional_movies = []
    for page in range(1, 3):  # Fetching movies from first 2 pages of TMDB popular movies
    ...
    # Combine liked movies + additional movies
    ...
    similar_movies = {}
    for idx, movie_id in enumerate(df['id'][:len(movie_data)]):  # Only compare liked movies
        ...
        for sim_idx, score in sim_scores:
            ...
            if similar_movie_id not in movie_ids:  # Exclude already liked movies
    ...
    print("Final Recommended Movies:", recommended_movies)  # Debugging step
...
def for_you_recommendations():
    ...
    return jsonify(movies_cleaned)  # Now JSON serializable


# Serve index2.html at the root URL
@app.route('/')
@login_required
def index():
    user_id = current_user.id
